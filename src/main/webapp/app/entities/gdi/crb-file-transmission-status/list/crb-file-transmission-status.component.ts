...
import { EntityArrayResponseType, CrbFileTransmissionStatusService } from '../service/crb-file-transmission-status.service';
...
export class CrbFileTransmissionStatusComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = [
    'submittedFileStatusTypeCode',
    'submittedFileStatusType',
  ...
  trackId = (_index: number, item: ICrbFileTransmissionStatus): number =>
    this.crbFileTransmissionStatusService.getCrbFileTransmissionStatusIdentifier(item);

  search(query: string): void {
    if (query && CrbFileTransmissionStatusComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (CrbFileTransmissionStatusComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.crbFileTransmissionStatuses = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ICrbFileTransmissionStatus[] | null): ICrbFileTransmissionStatus[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.crbFileTransmissionStatusService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.crbFileTransmissionStatusService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
