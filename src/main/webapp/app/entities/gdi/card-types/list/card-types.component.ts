...
import { EntityArrayResponseType, CardTypesService } from '../service/card-types.service';
...
export class CardTypesComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['cardTypeCode', 'cardType', 'cardTypeDetails'];
  ...
  trackId = (_index: number, item: ICardTypes): number => this.cardTypesService.getCardTypesIdentifier(item);

  search(query: string): void {
    if (query && CardTypesComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (CardTypesComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: ICardTypes[] | null): ICardTypes[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.cardTypesService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.cardTypesService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
