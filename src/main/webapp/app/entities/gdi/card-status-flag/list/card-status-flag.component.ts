...
import { EntityArrayResponseType, CardStatusFlagService } from '../service/card-status-flag.service';
...
export class CardStatusFlagComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['cardStatusFlag', 'cardStatusFlagDescription', 'cardStatusFlagDetails'];
  ...
  trackId = (_index: number, item: ICardStatusFlag): number => this.cardStatusFlagService.getCardStatusFlagIdentifier(item);

  search(query: string): void {
    if (query && CardStatusFlagComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (CardStatusFlagComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.cardStatusFlags = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ICardStatusFlag[] | null): ICardStatusFlag[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.cardStatusFlagService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.cardStatusFlagService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
