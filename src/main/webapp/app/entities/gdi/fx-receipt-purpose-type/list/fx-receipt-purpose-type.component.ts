...
import { EntityArrayResponseType, FxReceiptPurposeTypeService } from '../service/fx-receipt-purpose-type.service';
...
export class FxReceiptPurposeTypeComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFxReceiptPurposeType): number =>
    this.fxReceiptPurposeTypeService.getFxReceiptPurposeTypeIdentifier(item);

  search(query: string): void {
    if (query && FxReceiptPurposeTypeComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (FxReceiptPurposeTypeComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.fxReceiptPurposeTypes = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IFxReceiptPurposeType[] | null): IFxReceiptPurposeType[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.fxReceiptPurposeTypeService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.fxReceiptPurposeTypeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
