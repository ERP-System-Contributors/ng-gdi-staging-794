...
import { EntityArrayResponseType, CardCategoryTypeService } from '../service/card-category-type.service';
...
export class CardCategoryTypeComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['cardCategoryFlag', 'cardCategoryDescription', 'cardCategoryDetails'];
  ...
  trackId = (_index: number, item: ICardCategoryType): number => this.cardCategoryTypeService.getCardCategoryTypeIdentifier(item);

  search(query: string): void {
    if (query && CardCategoryTypeComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (CardCategoryTypeComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.cardCategoryTypes = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ICardCategoryType[] | null): ICardCategoryType[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.cardCategoryTypeService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.cardCategoryTypeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
